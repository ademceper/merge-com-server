...
export class Maily {
  ...
  setTheme(theme: Partial<ThemeOptions>) {
    this.config.theme = deepMerge(this.config.theme || DEFAULT_THEME, theme) as Partial<ThemeOptions>;
  }
  ...
  async render({ noHtmlWrappingTags, ...options }: RenderOptions = DEFAULT_RENDER_OPTIONS): Promise<string> {
    ...
    return reactEmailRenderAsync(markup, options as Parameters<typeof reactEmailRenderAsync>[1]);
  }
