...
type BooleanFlagKey = `IS_${Uppercase<string>}_ENABLED` | `IS_${Uppercase<string>}_DISABLED`;
type NumericFlagKey = `${Uppercase<string>}_NUMBER`;

type FlagKey = BooleanFlagKey | NumericFlagKey;

type FlagType<T> = T extends BooleanFlagKey ? boolean : T extends NumericFlagKey ? number : never;
 ...
 */
function testFlagEnumValidity<TEnum extends IFlags, IFlags = Record<FlagKey, FlagKey>>(
