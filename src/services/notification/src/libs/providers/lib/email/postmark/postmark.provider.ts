  ...
  private createMailData(options: IEmailOptions): Message {
    const mailData: Message = {
      ...
      Attachments: options.attachments?.map(
        (attachment) =>
          new Models.Attachment(attachment.name ?? 'attachment', attachment.file?.toString('base64') ?? '', attachment.mime, attachment.cid)
      ),
    };
...
const getFormattedTo = (to: string | string[] | undefined): string => {
  if (!to) return '';
