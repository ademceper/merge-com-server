...
import type { MessagesSendObject, UsersExportIdsObject, UsersExportIdsResponse } from 'braze-api';
...
export class BrazeEmailProvider extends BaseProvider implements IEmailProvider {
  ...
  private async mapToExternalID(options: string[]): Promise<string[]> {
    ...
    for (const email of options) {
      ...
      externalIds.push(...response.users.map((user) => user.external_id).filter((id): id is string => !!id));
    }
    ...
    const messageBody: MessagesSendObject = {
      ...
      messages: {
        email: {
          ...
          plaintext_body: options.text ?? undefined,
