...
import { SubscriberEntity, SubscriberRepository } from 'libs/dal';
...
export class UpdateSubscriber {
  ...
  public async execute(command: UpdateSubscriberCommand): Promise<SubscriberEntity> {
    ...
    if (command.email !== undefined) {
      updatePayload.email = command.email ?? undefined;
    }

    if (command.phone !== undefined) {
      updatePayload.phone = command.phone ?? undefined;
    }

    if (command.firstName !== undefined) {
      updatePayload.firstName = command.firstName ?? undefined;
    }

    if (command.lastName !== undefined) {
      updatePayload.lastName = command.lastName ?? undefined;
    }

    if (command.avatar !== undefined) {
      updatePayload.avatar = command.avatar ?? undefined;
    }

    if (command.locale !== undefined) {
      updatePayload.locale = command.locale ?? undefined;
    }

    if (command.timezone !== undefined) {
      updatePayload.timezone = command.timezone ?? undefined;
    }

    if (command.data !== undefined) {
      updatePayload.data = command.data ?? undefined;
    }
