...
export class TierRestrictionsValidateUsecase {
  ...
  async execute(command: TierRestrictionsValidateCommand): Promise<TierRestrictionsValidateResponse> {
    ...
    if (!stepType || !isDigestDelayOrThrottle(stepType)) {
      return [];
    }
    ...
    if (stepType !== StepTypeEnum.THROTTLE && command.cron && isCronExpression(command.cron)) {
    ...
    if (stepType !== StepTypeEnum.THROTTLE && isRegularDeferAction(command)) {
      ...
      if (deferDurationMs === null || deferDurationMs < MIN_VALIDATION_LIMITS.DEFER_DURATION_MS) {
        return [];
      }
    ...
    if (stepType === StepTypeEnum.THROTTLE && isRegularThrottleAction(command)) {
      ...
      if (throttleDurationMs === null || throttleDurationMs < MIN_VALIDATION_LIMITS.DEFER_DURATION_MS) {
        return [];
      }
...
function isRegularThrottleAction(command: TierRestrictionsValidateCommand) {
  return command.amount && command.unit && (!command.cron || !isCronExpression(command.cron));
}

function calculateThrottleDuration(command: TierRestrictionsValidateCommand): number | null {
  if (command.amount === undefined || command.amount === null || !command.unit) {
    return null;
  }
