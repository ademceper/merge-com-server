...
export class ConditionsFilter extends Filter {
  ...
  public async filter(command: ConditionsFilterCommand): Promise<IConditionsFilterResponse> {
    const variables = command.variables ?? ({} as IFilterVariables);
  ...
  private async processPreviousStep(
    ...
    const value = [PreviousStepTypeEnum.SEEN, PreviousStepTypeEnum.UNSEEN].includes(filter.stepType)
      ? message.seen
      : message.read;
    const passed: boolean = [PreviousStepTypeEnum.UNREAD, PreviousStepTypeEnum.UNSEEN].includes(filter.stepType)
      ? value === false
      : !!value;
  ...
  private async processIsOnline(
    ...
    if (hasNoOnlineFieldsSet) {
      filterProcessingDetails.addCondition({
        ...
        operator: filter.on === FilterPartTypeEnum.IS_ONLINE ? FieldOperatorEnum.EQUAL : (filter as IOnlineInLastFilterPart).timeOperator,
        passed: false,
      });
    ...
    const onlineInLastFilter = filter as IOnlineInLastFilterPart;
    const diff = differenceIn(currentDate, lastOnlineAt, onlineInLastFilter.timeOperator);
    const result = !!subscriber?.isOnline || (!subscriber?.isOnline && diff >= 0 && diff <= onlineInLastFilter.value);

    filterProcessingDetails.addCondition({
      filter: FILTER_TO_LABEL[filter.on],
      field: subscriber?.isOnline ? 'isOnline' : 'lastOnlineAt',
      expected: subscriber?.isOnline ? 'true' : `${onlineInLastFilter.value}`,
      actual: `${subscriber?.isOnline ? 'true' : diff}`,
      operator: onlineInLastFilter.timeOperator,
      passed: result,
    });
  ...
  private splitFilters(filter: StepFilter) {
    ...
    const webhookFilters = children.filter((childFilter) => childFilter.on === 'webhook');

    const otherFilters = children.filter((childFilter) => childFilter.on !== 'webhook');
  ...
  private async compileFilter(value: string, variables: IFilterVariables, job: JobEntity | undefined): Promise<string | undefined> {
    try {
