...
export class GetSubscriberTemplatePreference {
  ...
  async execute(command: GetSubscriberTemplatePreferenceCommand): Promise<ISubscriberPreferenceResponse> {
    ...
    const template = mapTemplateConfiguration({
      ...command.template,
      critical: subscriberWorkflowPreference.critical ?? false,
    });
  ...
  private async getSubscriberWorkflowPreference(
    ...
    const subscriberWorkflowChannels = subscriberWorkflowPreference
      ? GetPreferences.mapWorkflowPreferencesToChannelPreferences(subscriberWorkflowPreference.preferences)
      : {};
    const subscriberPreferenceType = subscriberWorkflowPreference?.type ?? PreferencesTypeEnum.SUBSCRIBER_WORKFLOW;
