...
export class TriggerEvent {
    ...
    try {
      ...
      if (!storedWorkflow && !command.bridgeWorkflow) {
        await this.createWorkflowTrace({
          ...
          workflowId: (storedWorkflow as NotificationTemplateEntity | null)?._id,
        });
  ...
  private async getMappedCommand(command: TriggerEventCommand, workflowId: string | undefined) {
    return {
      ...command,
  ...
  private mapTenant(tenant: TriggerTenantContext | null | undefined): ITenantDefine | null {
  ...
  private mapActor(subscriber: TriggerRecipientSubscriber | null | undefined): ISubscribersDefine | null {
  ...
  private async resolveContextKeys(command: TriggerEventCommand, workflowId: string | undefined): Promise<string[]> {
    if (!command.context) {
      return [];
    }
