...
import { ExecutionDetailsEntity, ExecutionDetailsRepository } from 'libs/dal';
import { ExecutionDetailsStatusEnum, FeatureFlagsKeysEnum } from 'libs/shared';
...
import type { EventType, StepRunTraceInput, TraceStatus } from '../../services/analytic-logs/trace-log';
...
export class CreateExecutionDetails {
  ...
  private async createTraceLogEntry(command: CreateExecutionDetailsCommand, createdAt: string): Promise<void> {
    ...
    const traceData: StepRunTraceInput = {
      ...
      user_id: '',
      subscriber_id: command._subscriberId || '',
      external_subscriber_id: command.subscriberId || '',
      event_type: eventType,
      title: command.detail,
      message: '',
      raw_data: command.raw || '',
