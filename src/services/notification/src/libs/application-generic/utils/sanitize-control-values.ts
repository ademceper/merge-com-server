...
function sanitizeInApp(controlValues: InAppControlType) {
  const normalized: Record<string, unknown> = {
    subject: sanitizeEmptyInput<string | undefined>(controlValues.subject),
    body: sanitizeEmptyInput<string | undefined>(controlValues.body),
    avatar: sanitizeEmptyInput<string | undefined>(controlValues.avatar),
...
function sanitizeEmail(controlValues: EmailControlType) {
  ...
  const emailControls: EmailControlType = {
    ...
    rendererType: controlValues.rendererType,
...
export function dashboardSanitizeControlValues(
  ...
  try {
    ...
    return normalizedValues as Record<string, unknown> & { skip?: Record<string, unknown> };
  } catch (error) {
    logger.error('Error sanitizing control values', error);

    return controlValues as Record<string, unknown> & { skip?: Record<string, unknown> };
  }
}
