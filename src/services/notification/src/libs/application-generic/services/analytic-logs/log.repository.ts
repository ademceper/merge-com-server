...
export abstract class LogRepository<TSchema extends ClickhouseSchema<any>, TEnhancedType> {
  ...
  private async initialize() {
    if ((process.env.NODE_ENV as string) !== 'local' && process.env.NODE_ENV !== 'test') {
      return;
    }
  ...
  private validateColumnName(columnName: SchemaKeys<TSchema> | string): void {
    if (!columnName || typeof columnName !== 'string') {
      throw new Error('Invalid column name: must be a non-empty string');
    }

    if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(columnName as string)) {
      throw new Error(`Invalid column name format: ${String(columnName)}`);
    }

    if (!this.schema.schema[columnName as string]) {
      throw new Error(`Column '${String(columnName)}' does not exist in schema`);
    }
  }
