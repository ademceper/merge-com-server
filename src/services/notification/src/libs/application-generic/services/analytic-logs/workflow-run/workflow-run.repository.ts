...
export class WorkflowRunRepository extends LogRepository<typeof workflowRunSchema, WorkflowRun> {
  ...
  async updateWorkflowRunState(
    ...
    try {
      ...
      const workflowRunData = this.mapNotificationToWorkflowRun(notification, workflow as NotificationTemplateEntity, {
        ...
        deliveryLifecycleDetail,
        userId: undefined,
        externalSubscriberId: notification.to?.subscriberId || undefined,
      });
  ...
  private mapNotificationToWorkflowRun(
    ...
    return {
      created_at: LogRepository.formatDateTime64(new Date(notification.createdAt ?? Date.now())),
      ...
      // Delivery lifecycle
      delivery_lifecycle_status: options.deliveryLifecycleStatus || ('' as unknown as DeliveryLifecycleStatusEnum),
      delivery_lifecycle_detail: options.deliveryLifecycleDetail || ('' as unknown as DeliveryLifecycleDetail),
