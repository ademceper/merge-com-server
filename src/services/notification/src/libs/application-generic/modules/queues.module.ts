...
const INTERNAL_MODULE_PROVIDERS: Provider[] = [memoryQueueService, featureFlagsService];
...
export class QueuesModule implements OnApplicationShutdown {
  static forRoot(entities: JobTopicNameEnum[] = []): DynamicModule {
    ...
    const healthIndicators: Type<any>[] = [];
    const tokenList: InjectionToken[] = [];
    const DYNAMIC_PROVIDERS: Provider[] = [...BASE_PROVIDERS];
