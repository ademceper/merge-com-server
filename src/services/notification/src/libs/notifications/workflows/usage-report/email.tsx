import { providers as sharedProviders } from 'libs/shared';
import {
  Body,
  Button,
  Column,
 ...
 * Renders a native <span> element with optional style and title.
 */
function Span({
  children,
  style,
...
const defaultDetailValueStyle: CSSProperties = {
  color: '#525866',
  fontWeight: 600,
};
...
export function Text({ style, children, title }: TextProps) {
  return (
    <Span style={{ ...defaultStyle, ...style }} title={title}>
      {children}
    </Span>
  );
}
...
const mediumNumberStyle: CSSProperties = {
  fontSize: '32px',
  fontWeight: 600,
...
const listValueCellStyle: CSSProperties = {
...
function ChangeBadge({ value, isUp }: { value: number; isUp: boolean }) {
  ...
  return (
    ...
    >
      <Row>
        <Column style={{ padding: '2px 4px 2px 4px', verticalAlign: 'middle' }}>
          <Img
            src={iconUrl}
            alt={isUp ? 'up' : 'down'}
            width={16}
...
function CardWithDetail({
  ...
  return (
    <Card>
      ...
      >
        <Row>
          <Column style={{ padding: '0 8px 0 0', verticalAlign: 'baseline' }}>
            <Span style={mediumNumberStyle}>{humanizeNumber(value)}</Span>
          </Column>
          <Column style={{ padding: '0', verticalAlign: 'baseline' }}>
