...
import type { AfterSuccessContext, AfterSuccessHook, BeforeCreateRequestHook, BeforeRequestContext, BeforeRequestHook, HookContext } from "./types.js";
...
export class NovuCustomHook
    ...
    async afterSuccess(_hookCtx: AfterSuccessContext, response: Response): Promise<Response> {
        ...
        if (jsonResponse && typeof jsonResponse === 'object' && Object.keys(jsonResponse).length === 1 && 'data' in jsonResponse) {
    ...
    private convertToRecord(headers: HeadersInit): Record<string, string> {
        if (Array.isArray(headers)) {
            ...
            return Object.fromEntries(Array.from((headers as any).entries()) as [string, string][]);
        } else {
