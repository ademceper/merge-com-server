...
export class ClientSDK {
  ...
  public _createRequest(
    ...
    for (const [k, v] of Array.from((securityHeaders as any).entries()) as [string, string][]) {
      headers.set(k, v);
    }
    ...
    for (const [k, v] of Array.from((userHeaders as any).entries()) as [string, string][]) {
      headers.set(k, v);
    }
...
async function logRequest(logger: Logger | undefined, req: Request) {
  ...
  for (const [k, v] of Array.from((req.headers as any).entries()) as [string, string][]) {
    logger.log(`${k}: ${v}`);
  }
  logger.groupEnd();
  ...
  switch (true) {
    ...
    case ct === 'multipart/form-data': {
      ...
      for (const [k, v] of Array.from((body as any).entries()) as [string, string | Blob][]) {
...
async function logResponse(logger: Logger | undefined, res: Response, req: Request) {
  ...
  logger.group('Headers:');
  for (const [k, v] of Array.from((res.headers as any).entries()) as [string, string][]) {
    logger.log(`${k}: ${v}`);
  }
  logger.groupEnd();
